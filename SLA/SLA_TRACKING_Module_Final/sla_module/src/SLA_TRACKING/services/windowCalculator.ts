...
import { createViolationIfNotExists } from "../repositories/violationRepo";
...
export async function computeServiceWindow(
  ...
  if (status === WindowStatus.BREACHED) {
    try {
      await createViolationIfNotExists(policy.id, windowRow.id, policy.targetPct, Number(uptimePct.toFixed(4)), null);
    } catch (err) {
