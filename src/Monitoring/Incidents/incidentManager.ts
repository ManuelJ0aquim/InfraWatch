...
export async function findOrOpenIncident(serviceId: string, reason?: string) {
  let incident = await prisma.incident.findFirst({ where: { serviceId, closedAt: null } });
  if (!incident) {
    incident = await prisma.incident.create({
      data: { serviceId, reason: reason ?? "auto-open", escalationLevel: 1 },
    });
  }
  return incident;
}
...
export async function closeIncidentIfRecovered(serviceId: string, recoveryConfirmations: number) {
  try {
    ...
    const openIncidents = await prisma.incident.findMany({ where: { serviceId, closedAt: null } });
...
export async function updateIncidentNotification(incidentId: string, retryCount: number, escalationLevel?: number) {
